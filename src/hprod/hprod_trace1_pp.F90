!######################################################################
real(c_double) function hprod_trace1_pp(dofc, lfield, den1, wfn, pwfn)
  ...
  twfn = 0.0
  call hprod_projpp(runit, lfield, wfn, twfn)
...
! Orimo_ECS
  ...
  if (dofc) then
     do ifun = 1, nfcore
        do l = abs(mval(ifun)), lmax1
           do irad = llrf, ulrf
              ttmp = ttmp + conjg(pwfn(irad, l, ifun)) * twfn(irad, l, ifun)
           end do
        end do
     end do
  end if
  ...
  do iact = 1, nact
     ...
     do jact = 1, nact
        ...
        if (mval(ifun) == mval(jfun)) then
           do l = abs(mval(ifun)), lmax1
              do irad = llrd, ulrd
                 ttmp = ttmp + conjg(pwfn(irad,l,ifun)) * twfn(irad,l,jfun) * den1(jact, iact)
              end do
           end do
        end if
     end do
  ...
  hprod_trace1_pp = aimag(tval)*2.0
  return

end function hprod_trace1_pp
!######################################################################
