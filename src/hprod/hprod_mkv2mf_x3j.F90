!######################################################################
subroutine hprod_mkv2mf_x3j()

  use, intrinsic :: iso_c_binding
  use mod_hprod, only : rho2, v2sph

  implicit none
  call hprod_mkv2mf_x3j_poisson('tot', rho2, v2sph)
  call hprod_mkv2mf_x3j_herm('tot', v2sph)

end subroutine hprod_mkv2mf_x3j
!######################################################################
subroutine hprod_mkv2mf_x3j_dyn()

  use, intrinsic :: iso_c_binding
  use mod_hprod, only : rho2, v2sph

  implicit none
  call hprod_mkv2mf_x3j_poisson('dyn', rho2, v2sph)
  call hprod_mkv2mf_x3j_herm('dyn', v2sph)
  call hprod_mkv2mf_x3j_poisson('fc1dyn', rho2, v2sph)
  call hprod_mkv2mf_x3j_herm('fc1dyn', v2sph)

!debug
!  write(6, "('hprod_mkv2mf_x3j_dyn: v2sph')")
!  call hprod_printrho(v2sph)
!debug

end subroutine hprod_mkv2mf_x3j_dyn
!######################################################################
subroutine hprod_mkv2mf_x3j_poisson(v2_type, rho2, v2sph)
...
!BUG dim = nradfc
     ...
     stop "bad v2_type in hprod_mkv2mf_x3j_poisson"
  end if

  !$omp parallel default(shared) private(lll, ull)
  !###########################
  call util_omp_disp(0, lmax2, lll, ull)
  call hprod_mkv2mf_x3j_poissonp(rho2, v2sph, dim, lll, ull, ifun_ll, ifun_ul, jfun_ll, jfun_ul)
!  call hprod_mkv2mf_x3j_poissonp2(rho2, v2sph, dim, lll, ull, ifun_ll, ifun_ul, jfun_ll, jfun_ul)
  !###########################
  !$omp end parallel

end subroutine hprod_mkv2mf_x3j_poisson
!######################################################################
subroutine hprod_mkv2mf_x3j_poissonp(rho2, v2sph, dim, lll, ull, ifun_ll, ifun_ul, jfun_ll, jfun_ul)
...
end subroutine hprod_mkv2mf_x3j_poissonp
!######################################################################
subroutine hprod_mkv2mf_x3j_poissonp2(rho2, v2sph, dim, lll, ull, ifun_ll, ifun_ul, jfun_ll, jfun_ul)
  ...
  real(c_double), allocatable :: rrho2(:,:,:)
  integer(c_int) :: ifun, jfun, l, m, irad, ld, info
  integer(c_int) :: ij, nij, mapij(1:2, 1:nfun*nfun)
...
end subroutine hprod_mkv2mf_x3j_poissonp2
!######################################################################
subroutine hprod_mkv2mf_x3j_herm(v2_type, v2sph)
  ...
  integer(c_int) :: l, ilat, ifun, jfun, irad, mfac
...
!BUG dim = nradfc
     ...
     stop "bad v2_type in hprod_mkv2mf_x3j_herm"
  end if

  !$omp parallel default(shared) private(llr, ulr, mfac)
  !###########################
  call util_omp_disp(1, dim, llr, ulr)
  do ifun = ifun_ll, ifun_ul
     do jfun = max(jfun_ll, ifun + 1), jfun_ul
        mfac = (-1)**(mval(ifun)-mval(jfun))
        do l = 0, lmax2
           do irad = llr, ulr
!              v2sph(irad, l, ifun, jfun) = conjg(v2sph(irad, l, jfun, ifun))
              v2sph(irad, l, ifun, jfun) = mfac*conjg(v2sph(irad, l, jfun, ifun))
           end do
        end do
     end do
  end do
...
end subroutine hprod_mkv2mf_x3j_herm
!######################################################################
