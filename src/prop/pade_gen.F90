!######################################################################
subroutine pade_gen(icomp, isplit, nrad, nsph, lmax, mmax, noffd, &
...
!
  implicit none
  integer(c_int), intent(in) :: icomp, isplit
...
!debug
  ...
  ld2 = 3 * nsub + 1
  if (icomp == 1) then
!     fac = - iunit * dtime / 2.d+0
     fac = - iunit * dtime
  else
!     fac = - runit * dtime / 2.d+0
     fac = - runit * dtime
  end if
  ...
  do l = 0, lmax
     ...
     if (info .ne. 0) then
        write(6, "('pade_gen-zgbtrf: l = ', i5, ' info = ', i5)") l, info
        stop 'Error of zgbtrf.'
     end if
!test     call zpotrf('L', dim, tinv(1,1,l), ld2, info)
!test     write(6, "('zpotrf: info = ', i5)") info
!     write(6, "('pade_gen: lu facts of 1 + i*T*dtime/2, l = ', i10, ' (real)')") l
!     do irad = 1, nrad - 1
!        jll = max(1, irad - nsub)
...
!        write(6, *)
!     end do
!     write(6, "('pade_gen: lu facts of 1 + i*T*dtime/2, l = ', i10, ' (imag)')") l
!     do irad = 1, nrad - 1
!        jll = max(1, irad - nsub)
!        jul = min(dim, irad + nsub)
!        do jrad = 1, nrad - 1
