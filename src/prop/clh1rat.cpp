		  ...
		  std::vector<dcomplex>& cd1)
{
  ...
  IO.read_info("h1rat_prodci_type", (long) 2, prodci_type);
  if (prodci_type < 0 || prodci_type > 2) abort();

  long len_piv = (Bas.GRad.nrad - 1) * (Bas.GAng.lmax1 + 1);
  long len_inv = (3 * Bas.GRad.nmax + 1) * (Bas.GRad.nrad - 1) * (Bas.GAng.lmax1 + 1);
		   ...
		   const clfield& Field, clhprod& HPW, clwfn& Wfn) const
{
  ...
  if (clcontrol::split_type == 0) {
    ...
    h1rat_prod_(&CTRUE, &CTRUE, lfield, &dtime, &dim_numer, &dim_denom, &ncoeff[0], &dcoeff1[0], &tpiv[0], &tinv[0], &Wfn.wfn[0]);
  }
}
//////////////////////////////////////////////////////////////////////////
...
{
  printf("# clh1rat::ncoeff %5ld\n", dim_numer);
  for (long inumer = 0; inumer <= dim_numer; inumer ++) {
    printf("%5ld%20.10e%20.10e\n", inumer, 
  	   real(ncoeff[inumer]), imag(ncoeff[inumer]));
  }
  printf("# clh1rat::dcoeff %5ld\n", dim_denom);
  for (long idenom = 0; idenom < dim_denom; idenom ++) {
    printf("%5ld%20.10e%20.10e%20.10e%20.10e\n", idenom, 
