!######################################################################
subroutine bas_pp_gen_oncvpsp()

  use, intrinsic :: iso_c_binding
  use mod_const, only : pi
  use mod_rad, only : nrad, xrad, wrad
  use mod_bas, only : pp_vloc, pp_pproj, pp_fproj, pp_gproj, pp_maxl, pp_nump, pp_irmax
  use mod_oncvpsp
  ...
  ! local part
  do irad = 1, nrad - 1
     pp_vloc(irad) = oncvpsp_getvloc(xrad(irad))
  end do

  ! nonlocal part
  pp_maxl = oncvpsp_maxl()
  do l = 0, pp_maxl
     pp_nump(l) = oncvpsp_nump(l)
     do iproj = 1, pp_nump(l)
        ...
        found = .false.
        do irad = 1, nrad - 1
           !pp_fproj(irad,iproj,l) = oncvpsp_getproj(iproj,l,xrad(irad)) * xrad(irad) * sqrt(wrad(irad))
           pp_fproj(irad,iproj,l) = oncvpsp_getproj(iproj,l,xrad(irad)) * sqrt(wrad(irad))
           pp_gproj(irad,iproj,l) = pp_fproj(irad,iproj,l) * oncvpsp_ekb(iproj,l)
           if (.not.found .and. &
  ...
  !      end do
  !   end do
  !   write(6,"(f15.10)") pp_vloc(irad)
  !end do
  !stop 'for debug @ bas_pp_oncvpsp'
  !debug

!  contains
  !######################################################################
  !######################################################################
end subroutine bas_pp_gen_oncvpsp
!######################################################################
