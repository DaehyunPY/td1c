!##########################################################
subroutine ccdt_l3p_1e_man03(i0)

! i0 ( i j k a b c )_yf + = 1 * P( 3 ) * Sum ( d ) 
!  * y ( i j k a b d )_y * f ( d c )_f 0
  ...
  do icc = 1,ncc3aaa
     ...
     do d = norb1+1,nact
        i0(i,j,k,a,b,c,1) = i0(i,j,k,a,b,c,1) &
             + g3inp(i,j,k,a,b,d,spin_g3aaa)*fock(d,c,spin_focka) &
             - g3inp(i,j,k,c,b,d,spin_g3aaa)*fock(d,a,spin_focka) &
             - g3inp(i,j,k,a,c,d,spin_g3aaa)*fock(d,b,spin_focka)
     end do
  end do
  !$omp end do
  !$omp do
  do icc = 1,ncc3aab
     ...
     do d = norb1+1,nact
        i0(i,j,k,a,b,c,2) = i0(i,j,k,a,b,c,2) &
             + g3inp(i,j,k,a,b,d,spin_g3aab)*fock(d,c,spin_focka) &
             + g3inp(i,j,k,d,b,c,spin_g3aab)*fock(d,a,spin_focka) &
             + g3inp(i,j,k,a,d,c,spin_g3aab)*fock(d,b,spin_focka)
     end do
  end do
  !$omp end do
  !$omp end parallel
...
end subroutine ccdt_l3p_1e_man03
!##########################################################
