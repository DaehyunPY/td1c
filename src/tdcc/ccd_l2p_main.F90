!**********************************************************
subroutine ccd_l2p_main()

  use, intrinsic :: iso_c_binding
  use mod_ormas,only : nact,nelact
  use mod_cc,only : norb1,cc_code,g2out
  use mod_cc2

  implicit none
!debug
!write(6,"('ccdt_l2p_main: skip l2.')")
!return
!debug

  if (cc_code(1:4) == 'auto') then
     if (nelact(1)>=2) then
        call ccd_l2_1(1,1,1,1,g2out(1,1,norb1+1,norb1+1,1))
        call ccd_l2_2(1,1,1,1,g2out(1,1,norb1+1,norb1+1,1))
        call ccd_l2_3(1,1,1,1,g2out(1,1,norb1+1,norb1+1,1))
        call ccd_l2_4(1,1,1,1,g2out(1,1,norb1+1,norb1+1,1))
        call ccd_l2_5(1,1,1,1,g2out(1,1,norb1+1,norb1+1,1))
        call ccd_l2_6(1,1,1,1,g2out(1,1,norb1+1,norb1+1,1))
        call ccd_l2_7(1,1,1,1,g2out(1,1,norb1+1,norb1+1,1))
        call ccd_l2_8(1,1,1,1,g2out(1,1,norb1+1,norb1+1,1))
        call ccd_l2_9(1,1,1,1,g2out(1,1,norb1+1,norb1+1,1))
     end if
     if (nelact(2)>=1) then
        call ccd_l2_1(1,2,1,2,g2out(1,1,norb1+1,norb1+1,2))
        call ccd_l2_2(1,2,1,2,g2out(1,1,norb1+1,norb1+1,2))
        call ccd_l2_3(1,2,1,2,g2out(1,1,norb1+1,norb1+1,2))
        call ccd_l2_4(1,2,1,2,g2out(1,1,norb1+1,norb1+1,2))
        call ccd_l2_5(1,2,1,2,g2out(1,1,norb1+1,norb1+1,2))
        call ccd_l2_6(1,2,1,2,g2out(1,1,norb1+1,norb1+1,2))
        call ccd_l2_7(1,2,1,2,g2out(1,1,norb1+1,norb1+1,2))
        call ccd_l2_8(1,2,1,2,g2out(1,1,norb1+1,norb1+1,2))
        call ccd_l2_9(1,2,1,2,g2out(1,1,norb1+1,norb1+1,2))
     end if
  else
    !call ccdt_l2p_man01(g2out,cc_work1,cc_work2,cc_work3) ! merged with 6
    !call ccdt_l2p_man02(g2out,cc_work1,cc_work2,cc_work3) ! merged with 3
     call ccdt_l2p_man03(g2out,cc_work1,cc_work2,cc_work3)
     call ccdt_l2p_man04(g2out,cc_work1,cc_work2,cc_work3)
     call ccdt_l2p_man05(g2out,cc_work1,cc_work2,cc_work3)
     call ccdt_l2p_man06(g2out,cc_work1,cc_work2,cc_work3)
    !call ccdt_l2p_man07(g2out,cc_work1,cc_work2,cc_work3) ! L3
    !call ccdt_l2p_man08(g2out,cc_work1,cc_work2,cc_work3) ! L3
    !call ccdt_l2p_man09(g2out,cc_work1,cc_work2,cc_work3) ! merged with 10
     call ccdt_l2p_man10(g2out,cc_work1,cc_work2,cc_work3)
     call ccdt_l2p_man11(g2out,cc_work1,cc_work2,cc_work3)
    !call ccdt_l2p_man12(g2out,cc_work1,cc_work2,cc_work3) ! L3
    !call ccdt_l2p_man13(g2out,cc_work1,cc_work2,cc_work3) ! L3
    !call ccdt_l2p_man14(g2out,cc_work1,cc_work2,cc_work3) ! L3
    !call ccdt_l2p_man15(g2out,cc_work1,cc_work2,cc_work3) ! L3
    !call ccdt_l2p_man16(g2out,cc_work1,cc_work2,cc_work3) ! L3
    !call ccdt_l2p_man17(g2out,cc_work1,cc_work2,cc_work3) ! T3,L3
    !call ccdt_l2p_man18(g2out,cc_work1,cc_work2,cc_work3) ! T3,L3
  end if

end subroutine ccd_l2p_main
!**********************************************************
