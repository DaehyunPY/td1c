!##########################################################
subroutine ccdt_bmat2p_man02(i0,work1,work2)

!  i0 ( i a )_ydtt + = -1/2 * Sum ( j k b ) 
!  * i1 ( j k b i )_ydt * t ( a b j k )_t 1
  ...
  work1 = 0d0
  work2 = 0d0
  call ccdt_bmat2p_man02_1(work1,work2)
...
end subroutine ccdt_bmat2p_man02
!##########################################################
subroutine ccdt_bmat2p_man02_1(i1aa,i1ab)

! i1 ( i j a k )_ydt + = +1/2 * Sum ( l b c ) 
!  * y ( l i j b c a )_y * dt ( c b k l )_dt 0

  use mod_ormas,only : nact
  use mod_cc,only : norb1,g3inp,dt2inp
  use mod_cc2
     ...
     do b = norb1+1,nact
        do c = norb1+1,b-1
           i1aa(i,j,a,k) = i1aa(i,j,a,k) &
                + g3inp(l,i,j,b,c,a,spin_g3aaa)*dt2inp(c,b,k,l,spin_t2aa)
        end do
        do c = norb1+1,nact
           i1aa(i,j,a,k) = i1aa(i,j,a,k) &
                + g3inp(i,j,l,c,a,b,spin_g3aab)*dt2inp(c,b,k,l,spin_t2ab)
        end do
     end do
     end do
  end do
     ...
     do b = norb1+1,nact
        do c = norb1+1,b-1
           i1ab(i,j,a,k) = i1ab(i,j,a,k) &
                - g3inp(l,j,i,b,c,a,spin_g3aab)*dt2inp(c,b,k,l,spin_t2aa)
        end do
        do c = norb1+1,nact
           i1ab(i,j,a,k) = i1ab(i,j,a,k) &
                - g3inp(l,i,j,b,a,c,spin_g3aab)*dt2inp(c,b,k,l,spin_t2ab)
        end do
     end do
     end do
  end do
...
end subroutine ccdt_bmat2p_man02_1
!##########################################################
